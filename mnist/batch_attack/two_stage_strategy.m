%------------------------------------------------------------%
% this script is used to produce Figure 2 and 4 in the paper %
%------------------------------------------------------------%
...
model_vec = {'madry_robust','mnist'};%'mnist','mnist_robust', 'cifar10'};
...
local_info_prefix = '../local_info';
bbox_result_prefix =  '../Results';
        ...
        for kk = 1:length(local_type_vec)
            ...
            for ii = 1:length(random_seed_vec)
                ...
                for k = 1:length(adv_type_vec)
                    ...
                    loc_model_num = size(pgd_cnt_vec,1);
                    failed_num = 1e10;
                    succ_model_num = np.zeros(1,size(pgd_cnt_vec,2));
                    for iii = 1:size(pgd_cnt_vec,2)
                        for j = 1:loc_model_num
                            if pgd_cnt_mat(iii,loc_model_num-j+1) < failed_num
