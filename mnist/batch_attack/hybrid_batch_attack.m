...
model_vec = {'madry_robust','imagenet','mnist_robust'};
% model_vec = {'cifar10_robust'};%'mnist','mnist_robust', 'cifar10'};
        ...
        for kk = 1:length(local_type_vec)
            ...
            for ii = 1:length(random_seed_vec)
                ...
                for k = 1:length(adv_type_vec)
                    ...
                    succ_vec_tmp = query_num_vec < max_query_num;
                    if sum(succ_vec_tmp) < sum(succ_rate_vec)
                        fprintf('there are some problems in the python code!\n');
                        iii = find(succ_vec_tmp<succ_rate_vec);
                        query_num_vec(iii) = max_query_num -1;
                    end
                    ...
                    pgd_cnt_name = ['pgd_cnt_mat_'  num2str(rand_num) '.txt'];
                    ...
                    % thanks to Jianfeng for running AutoZOOM in unnatural
                    % way, I have to deliberately process his files
                    trans_rate_vec = adv_img_loss == 0;    
                    if strcmp(adv_type,'no_tune')&& strcmp(model,'imagenet') && strcmp(attack_method,'autozoom')
                        query_num_vec(trans_rate_vec) = 1;
                    end
                    ...
                    orig_loss_name = ['succ_model_num_'  num2str(rand_num) '.txt'];
                    file_full_path=fullfile(file_path_head,orig_loss_name);
                    fileID = fopen(file_full_path);
                    formatSpec = '%f';
                    succ_model_num = fscanf(fileID,formatSpec);

                    file_name = ['max_gap_'  num2str(rand_num) '.txt'];
                    file_full_path=fullfile(file_path_head,file_name);
                    fileID = fopen(file_full_path);
                    formatSpec = '%f';
                    thres_vec_max_gap = fscanf(fileID,formatSpec);  

                    file_name = ['min_gap_'  num2str(rand_num) '.txt'];
                    file_full_path=fullfile(file_path_head,file_name);
                    fileID = fopen(file_full_path);
                    formatSpec = '%f';
                    thres_vec_min_gap = fscanf(fileID,formatSpec);  

                    file_name = ['ave_gap_'  num2str(rand_num) '.txt'];
                    file_full_path=fullfile(file_path_head,file_name);
                    fileID = fopen(file_full_path);
                    formatSpec = '%f';
                    thres_vec_ave_gap = fscanf(fileID,formatSpec); 
                    if strcmp(model,'mnist') || strcmp(model,'mnist_robust') || strcmp(model,'imagenet_bandit')
