...
def coordinate_ADAM(losses, indice, grad, hess, batch_size, mt_arr, vt_arr, real_modifier, lr, adam_epoch, beta1, beta2, proj):
    ...
    assert np.all(np.sqrt(1 - np.power(beta2,epoch))) >= 0
    corr = (np.sqrt(1 - np.power(beta2,epoch))) / (1 - np.power(beta1, epoch))
    m = real_modifier.reshape(-1)
    old_val = m[indice] 
    assert np.all(vt) >= 0, "%f,%s"%(vt, type(vt))
...
def ADAM(losses, indice, grad, hess, batch_size, mt_arr, vt_arr, real_modifier, lr, adam_epoch, beta1, beta2, proj, beta, z, q=1):
    ...
    print("Shape of z[0]",z[0].shape)
    print("Shape of grad[0]",grad[0].shape)
    print("real_modifier_shape",real_modifier.shape)
    """
    for i in range(q):
...
class blackbox_attack:
    def __init__(self, sess, model, args,num_channels,image_size,num_labels):
        ...
        self.assign_timg1 = tf.placeholder(tf.float32, self.image_shape)
        ...
        if args['dist_metric'] == 'l2':
            if self.USE_TANH:
                self.l2dist = tf.reduce_sum(tf.square(self.newimg-tf.tanh(self.timg1)/2), [1,2,3])
            else:
                self.l2dist = tf.reduce_sum(tf.square(self.newimg - self.timg1), [1,2,3])
            ...
            # distortion to the input data
            if self.USE_TANH:
                self.l2dist = tf.reduce_max(tf.abs(self.newimg-tf.tanh(self.timg1)/2),[1,2,3])
            else:
                self.l2dist = tf.reduce_max(tf.abs(self.newimg - (self.timg1)),[1,2,3])
    ...
    def attack(self, img, lab,orig_img):
        ...
        #success_status_vec = np.zeros(self.MAX_ITER+1)
        ...
        self.eval_costs = 0
        #best_eval_cost = 1e10# updated when any valid samples are found...
        ...
        for iteration in range(self.MAX_ITER):
            ...
            if iteration  == 0:
                ...
                loss, l2,real, other, loss1, loss2 = self.sess.run((self.loss,self.l2dist,
                    self.real,self.other,self.loss1,self.loss2), feed_dict=feed_dict)

            """
            print("**************Value of previous adam optimizer states:******************")
            print("mt:",self.mt)
            print("vt:",self.vt)
            print("adam epoch:",self.adam_epoch)
            print("real modifier:",self.real_modifier)
            """
            # perform the attack 
            ...
            """
            newimg,lower, upper,orig_img1 = self.sess.run((self.newimg,self.lower,self.upper,self.timg1),feed_dict={self.modifier: self.real_modifier})
            #print("max of clipped points",np.amax(newimg_tmp))
            print("max of orig img difference is:",np.amax(newimg))
            print("max of orig img difference is:",np.amax(np.tanh(orig_img1)/2))
            print("max difference of newimg and orig img:",np.amax(np.abs((newimg)-np.tanh(orig_img1)/2)))
            print("max difference of lower and orig img:",np.amax(np.abs(lower-np.tanh(orig_img1)/2)))
            print("max difference of upper and orig img:",np.amax(np.abs(upper-np.tanh(orig_img1)/2)))
            print("max of lower and upper",np.amax(lower),np.amax(upper))

            normalized_input = self.sess.run(self.normalized_input,feed_dict = {self.modifier:self.real_modifier})
            print("norm of normalized instance",np.sum(normalized_input**2)**.5)
            sys.exit(0)
            """
            ...
            #modified by Suya, store tar class probability...
            ...
            if l2 < C + C/100 and loss1 == 0.0:
                print("the value of l2 dist is", l2)
                print("Value of loss1 is:", loss1)
                print("real and other value is:", real, other)
                print("Early stopping because valid sample is found within threshold with {} queries".format(self.eval_costs))
                print("Early stopping because valid sample is found within threshold with {} optimization iterations".format(
                        iteration+1))
                #if self.eval_costs < best_eval_cost and self.eval_costs != 0:
                #best_eval_cost = self.eval_costs
                o_bestattack = nimg  # also updating the value of images and const
                o_best_const = CONST
                #success_status_vec[iteration+1:self.MAX_ITER+1] = 1
                break
        ...
        return o_bestattack,self.eval_costs
...
class ZOO_RV(blackbox_attack):
    ...
    def blackbox_optimizer(self, iteration):
        ...
        losses, l2s, loss1, loss2, scores, nimgs, real, other = self.sess.run([self.loss, self.l2dist, self.loss1, self.loss2, self.output, self.newimg,self.real,self.other], feed_dict=feed_dict) 
