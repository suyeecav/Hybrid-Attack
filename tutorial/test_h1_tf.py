...
from utils_tf import ImageNetProducer, DataSpec, compute_cw_loss, keras_model_wrapper, load_local_model
from attack_local_ensemble_tf import LinfPGDAttack
...
parser.add_argument('--blackbox-attack', type=str, default="nes", help='for black-box attacks')
...
def main():
	...
	if args["blackbox_attack"] == "nes": 
		...
		autozoom_attack_graph = AutoZOOM(sess, target_model, args, decoder,
	...
	for idx in range(len(orig_images)):
		...
		if args["blackbox_attack"] == "nes": 
			_, num_queries, ae = nes_attack(sess, args, target_model, attack_seed, initial_img, target_class, class_num, IMAGE_SIZE)
		elif args["blackbox_attack"] == "autozoom": 
			ae, num_queries = autozoom_attack(autozoom_attack_graph, attack_seed, initial_img, target_y_one_hot)
		else:
			raise NotImplementedError
