    ...
    for i = 1:length(model_vec)
        ...
        for kk = 1:length(local_type_vec)
            ...
            for ii = 1:length(random_seed_vec)
                ...
                local_file_path_head = [prefix '/' attack_method '/' 'Results/' rand_seed '/'];
                ...
                for k = 1:length(adv_type_vec)
                    ...
                    % end
                    file_path_head1 = local_file_path_head;
                   ...
                   % store the cost info of individual runs
                    ...
                    if strcmp(rand_seed,'1234')
                        imagenet_query_vec = succ_non_trans_query_num_vec;
                    end
                end
...
figure;
plot(1:length(imagenet_query_vec),sort(imagenet_query_vec),'-m','LineWidth',2);

ylim([1 max(imagenet_query_vec)]);
xlim([1 length(imagenet_query_vec)]);
xticks([1 20 30 40 50 60 70 80 90 length(imagenet_query_vec)]);
%ytickformat('%.5d') 
